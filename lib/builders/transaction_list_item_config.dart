...
class SambazaTransactionListItemConfigBuilder
  ...
  SambazaTransactionListItemConfigBuilder()
      : super(
          group: (Transaction transaction, [SambazaModel listItem]) =>
          ...
          title: (Transaction transaction, [SambazaModel listItem]) =>
              transaction.transactionNumber.toString(),
          trailing: _buildTrailing,
        );

  static List<Widget> _buildLeading(Transaction transaction,
      ...
      <Widget>[
        Icon(
          ...
          color: Colors.white,
          semanticLabel: transaction.method == TransactionMethod.mpesa
              ? 'M-Pesa'
              : 'Bank Transfer',
        ),
        Text(transaction.method == TransactionMethod.mpesa ? 'M-PESA' : 'Bank',
            style: TextStyle(
              color: Colors.white,
              fontSize: 8,
            )),
  ...
  static Widget _buildTrailing(Transaction transaction,
      [SambazaModel listItem]) {
    return Column(
      children: <Widget>[
        Icon(
          transaction.status == TransactionStatus.approved
              ? Icons.done
              : Icons.schedule,
          color: transaction.status == TransactionStatus.approved
              ? Colors.green
              : Colors.cyan,
