...
class SambazaList extends StatelessWidget {
  ...
  SambazaList(this.list, {super.key, required this.groupBy, required this.limit, this.reverse = false})
  ...
  @override
  Widget build(BuildContext context) => list.isNotEmpty
      ? Column(
...
class _SambazaListItem extends StatelessWidget {
  ...
  _SambazaListItem(this.config)
      : assert(config.leading.length == 2),
        assert(config.subtitle.isNotEmpty);
